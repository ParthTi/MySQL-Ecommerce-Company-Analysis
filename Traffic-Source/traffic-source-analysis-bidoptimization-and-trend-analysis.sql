...
/* 7 Task 3c) Checking whether or not the bid changes impacted our volumes after April 15, 2012. */
...
-- 	WEEK(ws.created_at) AS wk,
    ...
    CASE
		WHEN ws.device_type = 'desktop' THEN COUNT(DISTINCT ws.website_session_id) ELSE NULL END AS desktop_sessions,
	CASE
		WHEN ws.device_type = 'mobile' THEN COUNT(DISTINCT ws.website_session_id) ELSE NULL END AS mobile_sessions
FROM
	website_sessions AS ws
WHERE
